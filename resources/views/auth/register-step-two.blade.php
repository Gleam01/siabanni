...
@section('content')
<form method="POST" action="{{ route('register.step.two') }}" aria-label="{{ __('Informations Générales') }}">
    @csrf

    <div class="form-group row">
        <label for="lastName" class="col-md-4 col-form-label text-md-right">{{ __('Nom :* ') }}</label>
        <div class="col-md-6">
          <input type="lastName" name="lastName" class="form-control{{ $errors->has('lastName') ? ' is-invalid' : '' }}" id="lastName" value="{{ old('lastName') }}" autofocus required >
    ...
    <div class="form-group row">
        ...
        <div class="col-md-6">
          ...
          <input type="hidden" name="training_id" class="form-control" value="{{ session('training_id') }}" required >
          <input type="hidden" name="cycle" class="form-control" value="{{ session('cycle') }}" required >
        </div>
    </div>
    ...
    <div class="form-group row">
      ...
      <button type="submit" class="btn btn-primary offset-md-2 col-md-2 offset-md-1">
        Suivant
